package com.jadmin.admin.system.core.jwt;
...
public class JwtUtil
{
    ...
    public UsernamePasswordAuthenticationToken getAuthentication(final String username, final String token)
    {
        ...
        final Collection<? extends GrantedAuthority> authorities =
                ...
                Arrays.stream(claims.get(this.jwtSetting.getAuthoritiesKey()).toString().split(",")).map(SimpleGrantedAuthority::new)
    ...
    public String sign(final String username, final Collection<? extends GrantedAuthority> grantedAuthorities)
    {
        ...
        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(this.jwtSetting.getSecurityKey());
        Key signingKey = new SecretKeySpec(apiKeySecretBytes, signatureAlgorithm.getJcaName());
        // 添加构成JWT的参数
        return Jwts.builder()
                // 设置用户名
                .setSubject(username).setIssuedAt(now) // 创建时间
                // 添加权限属性
                .claim(this.jwtSetting.getAuthoritiesKey(), authorities)
                // 设置失效时间
                .setExpiration(date).signWith(signatureAlgorithm, signingKey) // 估计是第三段密钥
                // 哈夫曼压缩
    ...
    private Jws<Claims> parseToken(final String token)
    {
        try
        {
            Jws<Claims> claimsJws = Jwts.parser().setSigningKey(this.jwtSetting.getSecurityKey())
